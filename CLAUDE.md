...
- **Frontend** reads `VITE_OWNER_EMAIL` + `VITE_OWNER_NAME` from env at build time (exported as `OWNER_EMAIL` / `OWNER_NAME` from `src/lib/supabase.ts`). Surfaced on the Pending and Denied screens as a mailto link + personable "<name> needs to approve you" copy. Both fall back gracefully when unset — empty email suppresses the mailto, missing name is derived from the email local-part (or "the admin").
- **SQL trigger** (`handle_new_user` in `supabase/migrations/0001_init.sql`) still hardcodes `scott.t.weaver@gmail.com` — that's the source of the auto-admin + auto-Weavers grant on first sign-in. Forkers must edit that line before applying migrations. Tracked as a follow-up: move into an `app_config(owner_email)` table seeded by a separate SQL one-liner post-deploy.
...
- **`version/3.16.1`** (launched 2026-05-24) — patch: anchored drive start to Baronets Trail (78753) instead of generic "Austin". From north Austin the first leg to Buc-ee's Luling runs ~65 min vs. 50 from a central start, so `DRIVE_DEPART_TIME` shifted 11:00 AM → 10:45 AM CDT and `DRIVE_STOPS[0]` got its name + time updated (`'Baronets Trail, Austin'`, `'10:45 AM'`, note `'Depart 78753…'`). Buc-ee's onward keep their existing clock times (11:50 AM → … → 3:55 PM condo arrival) since only the Austin→Luling leg lengthened. No schema change.
- **`version/3.16`** (launched 2026-05-24) — drive plan with realistic stops. Old `DRIVE_STOPS` reflected pure driving time (3h 40m door-to-door); recalculated with 30 min at Buc-ee's + 30 min at H-E-B + 15 min gas/bathroom (Refugio area) — new total is 4h 55m. `time` field on `DriveStop` switched from cumulative offset strings (`"0:50"`) to wall-clock strings (`"11:50 AM"`) since elapsed offsets get noisier as stop durations stack; clock times also make "when am I getting there" obvious without mental math. Three new constants in `src/lib/trip-data.ts`: `DRIVE_DRIVING_HOURS = 3.5` (pure drive time kept separate from total elapsed), `DRIVE_DEPART_TIME = '11:00 AM CDT'`, `DRIVE_ARRIVAL_TARGET = '4:00 PM check-in'`. `DRIVE_DURATION_HOURS` bumped 3.5 → 5; subtitle in `DriveCard` reads "~5 hrs with stops · ~3.5 hrs driving · ~215 miles · …" with a second line for the departure recommendation. `DriveCard` dropped the `+` prefix on rendered stop times since they're no longer offsets. Recommended 11:00 AM departure lands the family at the condo ~3:55 PM, ~5 min before 4 PM check-in — noted in release notes that Memorial Day Monday ferry could eat that cushion and the conservative fallback is 10:45 AM. No schema change.
- **`version/3.15.2`** (launched 2026-05-20) — patch: dropped 🏖️ from browser tab title. `<title>Port A 2026 🏖️</title>` was rendering the emoji as a colored graphic in the tab strip alongside the (also-🏖️) favicon — duplicate icons, and v3.14's `(N)` prefix made the title longer so the trailing emoji got cut off more often. Now `<title>Port A 2026</title>`. The in-app header inside the dusk-gradient banner keeps the emoji — that's a styled element, not a tab-strip glyph. `useTabBadge` is unaffected (looks up the base title at runtime).
- **`version/3.15.1`** (launched 2026-05-20) — patch: License auto-detection. v3.15's LICENSE file had a dual-license header pasted above the canonical AGPL text, which threw off GitHub's Licensee detector (`gh api repos/... .license.spdx_id` returned `NOASSERTION` / "Other"). Split: LICENSE = pure canonical AGPL-3.0-or-later (gets auto-detected), NOTICE = the dual-license header content (standard convention for projects that need both). No legal-scope change. README structure updated to mention NOTICE.
- **`version/3.15`** (launched 2026-05-20) — formal dual licensing. No code change. New top-level docs: `LICENSE` (canonical FSF AGPL-3.0-or-later text with Scott T Weaver copyright header), `LICENSING.md` (entry-point guide with situation→license mapping), `COMMERCIAL-LICENSE.md` (plain-English summary of the paid annual alternative, contact <scott.t.weaver@gmail.com>), `COMMERCIAL-TERMS.md` (substantive terms outline, marked as placeholder pending legal review), `CLA.md` (Individual Contributor License Agreement) and `CLA-CORPORATE.md` (Corporate variant). README updated with top-level license note + new "Licensing & contributing" section + project structure. `package.json` license field set to `"AGPL-3.0-or-later OR SEE LICENSE IN COMMERCIAL-LICENSE.md"` plus `author` field. AGPL text is canonical and safe; commercial and CLA docs are modeled on widely-adopted patterns (Apache ICLA, Plausible, Cal.com) but explicitly flagged in README + each doc as **not reviewed by legal counsel** — a reasonable starting point, not legal advice. Tracked follow-ups: CLA-bot integration via cla-assistant.io, and legal review of `COMMERCIAL-TERMS.md` when a real commercial deal materializes. SQL trigger owner-email cleanup (issue #1) remains the other open licensing-adjacent backlog item.
- **`version/3.14`** (launched 2026-05-20) — tab title unread badge. New `useTabBadge(count)` hook in `src/lib/useTabBadge.ts` prefixes `document.title` with `(N)` whenever `totalUnread > 0` (capped at `99+`). Pure DOM mutation — strips any existing `(N)\s+` prefix before applying the new one so successive count changes don't accumulate; cleanup on unmount restores the base. Wired into `Dashboard.tsx` next to the existing `totalUnread` useMemo. Useful when the tab is backgrounded and the in-app badges aren't visible. Favicon-dot support deferred as a possible follow-up (especially valuable on mobile where tab titles are basically invisible). No schema change.
- **`version/3.13`** (launched 2026-05-20) — forkable owner identity. `OWNER_EMAIL` / `OWNER_NAME` constants in `src/lib/supabase.ts` are now env-driven (`VITE_OWNER_EMAIL`, `VITE_OWNER_NAME`) so the deployed JS doesn't ship a personally-identifying email baked in for any fork of the repo. Both vars fall back gracefully: empty email suppresses the mailto rendering on Pending / Denied screens; missing name derives from the email local-part (`scott.t.weaver@…` → "Scott") then to "the admin". `PendingScreen` and `DeniedScreen` updated to use `OWNER_NAME` instead of hardcoded "Scott" and to guard the mailto when the email is empty. Test fixture in `format.test.ts` switched from Scott's email to `test.user@example.com`. README + CLAUDE.md updated. Known gap: SQL trigger `handle_new_user` in migration 0001 still hardcodes the email — that needs a separate migration (planned: `app_config(owner_email)` table) and is documented in the README fork instructions. No schema change in this release; Vercel prod env needs `VITE_OWNER_EMAIL` + `VITE_OWNER_NAME` added so existing users don't see the generic fallback copy.
- **`version/3.12.1`** (launched 2026-05-20) — patch: missing-first is family-scoped for quantity items. v3.12 shipped quantity-missing as "total across all families = 0", but that's wrong: another family bringing 6 towels doesn't mean MY family doesn't still need to bring some. Now mirrors task semantics — missing if MY family hasn't committed any (`getContribution(item.id, myFamilyId)?.quantity ?? 0 === 0`). Task and claim semantics unchanged. Fallback (rare, user with no family yet): use overall-coverage signal.
- **`version/3.12`** (launched 2026-05-20) — per-category sort. Each `ChecklistSection` gets a small Sort selector (above the items list, hidden when item count ≤ 1) with three modes: `default` (existing curator sort_order), `alphabetical` (locale-aware), `missing-first`. New `useSortMode` hook in `src/lib/useSortMode.ts` mirrors the `useCollapsedState` localStorage pattern (`port-a:sort:<userId>:<categoryKey>`) so each user's choice persists per-device. `isMissing()` helper in ChecklistSection switches on tracking type: quantity = total across families is 0; task = my family hasn't ticked it (per-family scope means another family's tick doesn't close mine); claim = nobody has claimed it (once anyone claims, it's covered for everyone — explicit user rule that claim items don't show as missing for families NOT bringing them). Default mode is a passthrough — no array copy in the common case. No schema change.
- **`version/3.11.1`** (launched 2026-05-20) — patch: Shop badge counts pending only. v3.11 shipped the badge showing total list size, but already-purchased items shouldn't drive a "you have something to act on" affordance. Dashboard now computes `shoppingPendingCount = Array.from(shopping.entries.values()).filter(p => !p).length` and passes that to TopBar; badge hides at 0. Tooltip phrasing updated to "N items still to buy" when there's a count.
- **`version/3.11`** (launched 2026-05-20) — shopping list mode + action-icon contrast. Migration `0010_shopping_list.sql` adds `shopping_list(item_id, family_id, purchased boolean, added_by, added_at, updated_at)` with composite PK and family-private RLS mirroring `packing_status`. The `purchased` column is the new bit — Pack didn't need it (unpacking = delete), but Shopping keeps the row around for the Need/Got toggle. New `useShoppingList` hook (cache → realtime → queue, family-scoped channel filter) with three queue ops: `upsert` / `updatePurchased` / `delete`. UI: every `ChecklistRow` gains a ShoppingBag icon button — tap to add (amber fill), tap again to remove; the Dashboard wraps it as a `toggleShopping` callback. New `ShoppingView` clones `PackView`'s Need-on-top + Got-collapsed pattern, plus an explicit X-to-remove per row (Pack doesn't need this since unpack just moves into the other section). Toolbar gets a Shop button next to Pack with an amber count badge when the list is non-empty. **Contrast pass**: every action button on `ChecklistRow` was `text-slate-300` which was nearly invisible against `bg-sand-50` — bumped to `text-slate-500`. Also removed the `opacity-0 group-hover:opacity-100 sm:opacity-100` dance on the hide + delete buttons since the icons are now contrast-correct without it. Same Circle-icon fix in PackView/ShoppingView (300 → 400).
- **`version/3.10`** (launched 2026-05-19) — on-island essentials card. Family member asked for "grocery + similar things you can grab without the ferry." New `ON_ISLAND_ESSENTIALS` array in `trip-data.ts` (interface `OnIslandEssential` mirrors the `Place` shape: emoji + category + name + address + phone + optional hours + note). 7 seeded entries researched from public web sources: Lowe's Market (formerly Family Center IGA, ownership changed 2025), Ace Hardware, Valero on SH-361, Island Tackle, CVS, Spanky's Liquor, Coffee Waves. New `OnIslandCard` between MealsCard and the checklists; groups by category for future entries; each card shows tap-to-call phone + tap-to-Google-Maps address. Static data on purpose — codes/wifi changed, but on-island businesses don't, so deploy-to-update is fine. Easy to move to a DB-backed admin-editable table later if the list grows. No schema change, no migration.
- **`version/3.9.1`** (launched 2026-05-19) — patch: hide-by-default codes. Door code, pool code, and wifi password on the CondoPortalCard now mask with `•` chars (matching the real value's length so the rendered tile stays the same width); each has an Eye/EyeOff toggle to reveal/hide. Per-tile local `useState`, default hidden on every page load (no persistence — security-by-default). `CodeTile` gets an optional `secret?: boolean` prop; wifi password uses a new inline `Secret` component with the same toggle pattern. `select-all` on the value spans so triple-click still copies the real value when revealed (handy for sharing the wifi password). No schema change.
- **`version/3.9`** (launched 2026-05-19) — condo info: DB-backed, admin-editable, realtime. Migration `0009_condo_info.sql` adds single-row `condo_info` table (PK enforced as `id = 1`) with door/pool codes, wifi, host contact, check-in/out times, bike + golf cart rental fields, and free-form notes. RLS: approved read, admin write. Seeded with the two rentals up front (Big Shell Bikes + Joy Carts). New `useCondoInfo` hook follows the standard cache→realtime→queue pattern (single-row variant; `.eq('id', 1)` everywhere, no list semantics). `CondoPortalCard` rewritten to render structured info (KeyRound/Waves code tiles up top, then wifi/host/times/rentals/notes, then the existing "Open guest portal" button at the bottom). Empty fields self-hide so the card grows as data is added. New `CondoInfoEditor` section in `AdminPanel`: dirty-diff Save (no churn for unchanged fields), realtime updates only overwrite the form when there are no unsaved local changes — fine at single-admin scale. Custom `CartIcon` SVG inline in CondoPortalCard since Lucide doesn't ship one. The TrackHS auto-login URL stays in `VITE_CONDO_PORTAL_URL` env per request — may move into `condo_info` later for full consolidation.
- **`version/3.8`** (launched 2026-05-19) — condo portal card. New `CondoPortalCard` between BookedActivity and MealsCard on the trip tab (inside the `!filterActive` block so it auto-hides under search). Reads `VITE_CONDO_PORTAL_URL` from env at build time — Vercel Production + local `.env.local`, intentionally NOT in `.env.example` or `.env.development.local`. The TrackHS auto-login URL is a bearer credential (anyone with the link gets in), so it stays out of the public GitHub repo entirely. Card renders a "Portal link not configured" hint when the env var is missing so prod doesn't crash on a forgotten var. No schema changes. Also established the `vercel:env` skill flow for env-var management — Vercel CLI is not installed locally, so this release's env var was added via the Vercel dashboard. Future env work can use `npx -y vercel env add` after a one-time `npx vercel login`.
...
- **`version/3.7`** (launched 2026-05-18) — live weather + tide data. Two new helper libs: `src/lib/openMeteo.ts` (Open-Meteo daily forecast for Port Aransas lat 27.8336 / lon -97.0608, fahrenheit, America/Chicago tz) and `src/lib/noaaTides.ts` (NOAA CO-OPS predictions for station 8775237, MLLW datum, English units, `interval=hilo`). Both are pure fetch + parse with 17 new unit tests covering the WMO weather-code → Condition mapping and the hi/lo grouping/best-time heuristic. New hooks `useWeather` and `useTides` use stale-while-revalidate: hydrate from localStorage cache on first render (`weather:open-meteo`, `tides:noaa`), fetch fresh on mount, refetch on reconnect + on tab-focus when cache age > 1h. AbortController cancels in-flight fetches. On any failure, falls back to the hardcoded `WEATHER_FORECAST` / `TIDE_FORECAST` constants in `trip-data.ts` (which stay as a permanent safety net, not deleted). New `FreshnessBadge` component shared by both cards shows "Updated Xm ago · Provider" with a manual refresh button; coral `CloudOff` indicator when on fallback. Refresh button has `e.stopPropagation()` to prevent it triggering the parent CollapsibleCard's toggle (same pattern as the unread-count pills in ChecklistSection). The two TODO items deferred from earlier ("Real weather data via Open-Meteo" / "Real NOAA tide data") are now done. No schema changes, no prod migration.
